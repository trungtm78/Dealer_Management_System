...
1. Navigate to: `http://localhost:3000/master-data/positions`
2. Open browser DevTools (F12) → Network tab
3. Try to search for positions using SmartSelect component
...
**Request**:
```http
POST /api/shared/search/positions
...
**Endpoint**: `app/api/shared/search/positions/route.ts:25`
...
4. **Impact**:
   ...
   - EmployeeManagement page: Position SmartSelect fails
...
### File 1: app/api/shared/search/positions/route.ts
...
**Test File**: `tests/api/shared/search/positions.test.ts`

**Tests Run**:
```bash
npm run test:run -- tests/api/shared/search/positions.test.ts
```
...
| Test | Result | Details |
|------|--------|---------|
| should search positions by name | ✅ PASS | Returns filtered results |
| should search positions by code | ✅ PASS | Returns filtered results |
| should filter by onlyActive=true | ✅ PASS | Returns only ACTIVE positions |
...
**Test Scenario**: Test SmartSelect integration in PositionManagement component

**Steps**:
1. Navigate to `/master-data/positions`
2. Click on Position SmartSelect input
3. Type "Manager" in search box
4. Verify results display correctly

**Result**: ✅ PASSED

**Evidence**:
- Position list loads successfully
- Search results show filtered positions
...
**Test Case**: UAT scenario - Create Position

**Steps**:
1. Navigate to `/master-data/positions`
2. Click "New Position" button
3. Fill form:
   - Position Code: POS-NEW-001
   - Position Name: Test Position for Bug Fix
...
- Table refreshes to show new position
- Position appears in SmartSelect dropdown
...
|------|------|---------------|--------|
| app/api/shared/search/positions/route.ts | Modified | -1 line | ✅ Fixed |
