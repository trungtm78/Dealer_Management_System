...
export async function POST(req: NextRequest) {
    try {
        ...
        if (q && q.trim()) {
            ...
            where.OR = [
                { level_name: { contains: query } },
                { level_code: { contains: query } }
            ];
        }

        const items = await prisma.master_levels.findMany({
            where,
            take: limit + 1,
        ...
        const selectItems: SelectItem[] = items.slice(0, limit).map((level: any) => ({
            id: level.id,
            label: level.level_name,
            subtitle: `${level.level_code} | ${level._count.employees} employees`,
            meta: level
        }));
        ...
        console.error("Error in employee-levels search:", error);
        return NextResponse.json(
            { error: "Failed to search employee-levels" },
            { status: 500 }
        );
    }
}
